...
## Very briefly - why do spatial analysis in R?
...
-   Different data sources will have different projections (e.g., Stats Canada uses NAD83 / Statistics Canada Lambert EPSG 3347)
-   Often a projection that uses meters as units is necessary for analysis, reproject your data before starting! 
...
install.packages(c('sf', 'terra', 'tidyterra', 'mapview', 'ggplot2', 
                   'dplyr', 'tidyr', 'tibble', 'osmdata', 'basemaps'))
...
mtlcrs <- 3347
rv_m <- st_transform(rv_cc, mtlcrs)
...
cls <- data.frame(id=1:5, cover=c("low_imp", "high_imp", "low_veg", "high_veg", "water"))
levels(cc) <- cls

ggplot() + 
  geom_spatraster(data = cc) + 
  scale_fill_manual(values = c('lightgrey', 'darkgrey', 'lightgreen', 
                               'darkgreen', 'darkblue'), na.value=NA) 
```
